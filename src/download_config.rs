...
pub fn generate_config_file(keyboard : Keyboard) -> String{
    ...
    for key_layer in keyboard.layer {
        for key_row in key_layer.chunks(keyboard.column.len()){
            proto_layer.push(format!("[{}]",key_row.iter().map(|key| format!("{}{}",key_prefix,key)).collect::<Vec<_>>().join(",")));
        }
    }
    let keymaps = format!("[{}]",proto_layer.join(",\n"));
